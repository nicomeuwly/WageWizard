...
export default function AuthInput(props: InputProps) {
    ...
    useEffect(() => {
        ...
        ref.current.style.paddingLeft = `${height * 1.2}px`;
        if (circle.current) {
            circle.current.style.height = `${height + 2}px`;
        }
    })
    return (
        <div className="w-3/4 static">
            ...
            <input
                className="box-border bg-gray-2 border border-font-secondary w-full rounded-full py-4 text-white focus:outline-none focus:border-yellow focus:ring-1 focus:ring-yellow focus:bg-transparent focus:placeholder-transparent caret-yellow"
                type={props.type}
                name={props.name}
