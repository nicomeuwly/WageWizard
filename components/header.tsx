...
export default function Header(props: { title: string, leftButton: { icon: string, link: string }, rightButton: { icon: string, link: string } }) {
    ...
    return (
        <header className="bg-gray p-5 w-full border-t-4 border-yellow flex flex-row shadow-lg shadow-black/50">
            <Link href={props.leftButton.link} className={"text-left align-middle text-font-secondary " + (!displayLeftButton ? "pointer-events-none hidden" : "basis-[20%]")}><span className="material-symbols-rounded aspect-square hover:bg-gray-2 rounded-full">{props.leftButton.icon}</span></Link>
