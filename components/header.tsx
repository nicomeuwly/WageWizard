...
export default function Header(props: { title: string, leftButton: { icon: string, link: string }, rightButton: { icon: string, link: string } }) {
    ...
    return (
        <header className="bg-gray p-5 w-full border-t-4 border-yellow flex flex-row shadow-lg shadow-black/50">
            <Link href={props.leftButton.link} className={"text-left align-middle text-font-secondary " + (!displayLeftButton ? "pointer-events-none hidden" : "basis-[20%]")}><span className="material-symbols-rounded h-full align-middle aspect-square p-1 hover:bg-gray-2 rounded-full">{props.leftButton.icon}</span></Link>
            ...
            <Link href={props.rightButton.link} className={"text-right align-middle text-font-secondary " + (!displayRightButton ? "pointer-events-none hidden" : "basis-[20%]")}><span className="material-symbols-rounded h-full align-middle aspect-square p-1 hover:bg-gray-2 rounded-full">{props.rightButton.icon}</span></Link>
        </header>
    )
}